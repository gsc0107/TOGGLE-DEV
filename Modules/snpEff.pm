...
#sub snpEffdbCreator { #will prepare a snpEff database 
...
sub snpEffAnnotation { #Will annotate a VCF file based on a already prepared db for SNPeff
    my ($vcf,$outputName, $optionsHachees)=@_;
    ...
    my $command=$snpEff." ann ".$options." ".$vcf." > ".$outputName;
...
=head1 SYNOPSIS
	...
	snpEff::snpEffAnnotation($vcf,$outputName, $optionsHachees)
