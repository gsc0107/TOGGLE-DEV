...
sub transabyssRun
{
    ...
    {
        toolbox::exportLog("ERROR: transabyss::transabyssRun : You do not need to specify name or outdir options\n",2);
    }
    ...
    if (scalar (@{$secondListOfFastq}) > 0) # if is not empty,we have paired reads
    {
        if ((scalar (@{$firstListOfFastq}) > 0 ) and (scalar (@{$firstListOfFastq}) == scalar (@{$secondListOfFastq})) )  # if is not empty and same length
        {
            ...
            {
                if (@{$firstListOfFastq}[$i] ne "NA" and toolbox::checkFormatFastq(@{$firstListOfFastq}[$i])==1 )
                { ##Check if entry file exist and is not empty
                    if (@{$secondListOfFastq}[$i] ne "NA" and toolbox::checkFormatFastq(@{$secondListOfFastq}[$i])==1 )
                    { ##Check if entry file exist and is not empty
                    ...
                    {
                        toolbox::exportLog("ERROR: transabyss::transabyssRun : The file ".@{$secondListOfFastq}[$i]." is uncorrect or is not a Fastq\n",0);
                        return 0;#File not Ok
                    }
                }
                else
                {
                    toolbox::exportLog("ERROR: transabyss::transabyssRun : The file ".@{$firstListOfFastq}[$i]." is uncorrect or is not a Fastq\n",0);
                    return 0;#File not Ok
                }
            }   
    ...
    #print $command,"\n";
    #toolbox::exportLog("xxxxxxxxxxxxxxxRUN: transabyss::transabyssRun : CMD:: $command\n",0);
    #Execute command
    ...
    {
         toolbox::exportLog("ERROR: transabyss::transabyssRun : Uncorrectly done\n",0);
