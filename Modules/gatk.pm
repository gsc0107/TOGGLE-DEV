...
sub gatkBaseRecalibrator
{
    ...
    if ((checkFormat::checkSamOrBamFormat($bamToRecalibrate)==2) and (toolbox::sizeFile($refFastaFileIn)==1) and (toolbox::sizeFile($bamToRecalibrate)==1))     # check if files exists and arn't empty and stop else
    {
...
sub gatkRealignerTargetCreator
{
    ...
    if ((checkFormat::checkSamOrBamFormat($bamToRealigne)==2) and (toolbox::sizeFile($refFastaFileIn)==1) and (toolbox::sizeFile($bamToRealigne)==1))     # check if files exists and arn't empty and stop else
    {
        my $options=toolbox::extractOptions($optionsHachees);       # extraction of options parameters
        if ($options !~ m/-T/) # The type of walker is not informed in the options
        {
...
sub gatkIndelRealigner
{
    ...
    if ((checkFormat::checkSamOrBamFormat($bamToRealigne)==2) and (toolbox::sizeFile($refFastaFileIn)==1) and (toolbox::sizeFile($bamToRealigne)==1) and (toolbox::readFile($intervalsFile)==1))      # check if files exists and arn't empty and stop else
    {
        my $options=toolbox::extractOptions($optionsHachees);       # extraction of options parameters
        if ($options !~ m/-T/) # The type of walker is not informed in the options
        {
...
sub gatkUnifiedGenotyper
{
    ...
    foreach my $file (@{$listOfBam})       # for each BAM file(s)
    {
        if (checkFormat::checkSamOrBamFormat($file)==2 and toolbox::sizeFile($file)==1)        # if current file is not empty and is a BAM
        {
...
sub gatkReadBackedPhasing
{
    ...
    #TODO adding VCF type control
    if ((checkFormat::checkSamOrBamFormat($bamFileIn)==2) and (toolbox::sizeFile($refFastaFileIn)==1) and (toolbox::sizeFile($bamFileIn)==1) and (toolbox::sizeFile($vcfVariant)==1))     # check if ref file exist and isn't empty and stop else
    {
        my $options=toolbox::extractOptions($optionsHachees);
        if ($options !~ m/-T/) # The type of walker is not informed in the options
        {
