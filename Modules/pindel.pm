...
our $nameConfig="../../DATA/testData/samBam/samBamSV/configSV/pindelConfFile.txt";


sub pindelRun
{
     my($pindelConfigFile,$pindelFileOut,$reference,$listOfBam,$optionsHachees)=@_;
     my $command="";
     my $options="";
     if ($pindelConfigFile ne "" and toolbox::sizeFile($pindelConfigFile)==1)
     { ##Check if entry file exist and is not empty
          if ($optionsHachees) {
               $options=toolbox::extractOptions($optionsHachees); ##Get given options
          }
          $command=$pindel." ".$options." -i ".$pindelConfigFile." -f ".$reference." -o ".$pindelFileOut;
          #toolbox::exportLog($command."\n",1);
     }
     else
     {
          pindelConfig($listOfBam);
          $command=$pindel." ".$options." -i ".$nameConfig." -f ".$reference." -o ".$pindelFileOut;
...
sub pindelConfig
{
     my ($listOfBam) = @_;
     ##DEBUG my $line;
     open (confFile, ">$nameConfig") || die ("You can't create the file \"$nameConfig\"");
     foreach my $file (@{$listOfBam})       # for each BAM file(s)
     {
          ...
          {
              ##DEBUG $line.=$file."\n";
               my $insertSize = `samtools view -f2 $file | cut -f 9 | grep -v "-" | grep -v "^0" | head -n 1000000 | sort -k1n,1n | awk '{t+=\$1} END {print t/NR}'`;
               print confFile "$file\t$insertSize\t$file\n";
     ...
     close (confFile);
     ##DEBUG toolbox::exportLog("$line", 1);       # recovery of informations fo command line used later
}
...
#     }
#     close (confFile);
#     toolbox::exportLog("$line", 1);       # recovery of informations fo command line used later
#}
#
#1;